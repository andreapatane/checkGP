...
if nargin < 11
    %making lower approximation of K(gamma)
    [a_lower, b_lower, a_upper, b_upper] = linear_bounds_on_K_of_gamma(gamma_l,gamma_u);
    
    
    %taking into account kernel hyperparameters
    %k1 = sigma_b_2;
...
% taking into accounts linear coefficients on dot product
% removing so that approximation is on beta
% % % a_alpha = sigma_b_2/(sigma_b_2 + sigma_w_2/m);
% % % b_alpha = (sigma_w_2/m)/(sigma_b_2 + sigma_w_2/m);
% % % 
% % % a_i = a_i + b_i * a_alpha;
% % % b_i = b_i * b_alpha;
